...
class TestWordTokenize(TestCase):
    ...
    def setUpClass(cls):
        cls.sample_sentence = "The sky above the port was the color of television, tuned to a dead channel."
        cls.sample_numbers = "1, 2, 3,"
        cls.sample_contraction = "don't"
    ...
    def test_get_words(self):
        """It should return a list of words, ignoring punctuation.

        """
        from tokenizer import word_tokenize
        res = word_tokenize(self.sample_sentence)
        self.assertEqual(res, ['The', 'sky', 'above', 'the', 'port', 'was', 'the', 'color', 'of', 'television', 'tuned', 'to', 'a', 'dead', 'channel',])

    def test_get_numbers(self):
        """It should return a list of numbers, ignoring punctuation.

        """
        from tokenizer import word_tokenize
        res = word_tokenize(self.sample_numbers)
        self.assertEqual(res, ['1', '2', '3',])

    def test_get_contractions(self):
        """It should return an unmutilated contraction, ignoring punctuation.

        """
        from tokenizer import word_tokenize
        res = word_tokenize(self.sample_contraction)
        self.assertEqual(res, ["don't"])
